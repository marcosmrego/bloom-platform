...
-- Human editorial decisions are kept separate from automation executions.
...
-- Destino comercial verificável por post: produto, busca ou oferta temporária.
CREATE TABLE IF NOT EXISTS post_commerce (
    id BIGSERIAL PRIMARY KEY,
    tenant_id INT NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    post_id INT NOT NULL,
    link_type VARCHAR(20) NOT NULL CHECK (link_type IN ('product', 'search', 'offer')),
    product_id INT,
...
SELECT
    ...
    pc.valid_until AS offer_valid_until, pc.verified_at AS offer_verified_at,
