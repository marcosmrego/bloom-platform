...
cur.execute('UPDATE posts SET product_id=NULL WHERE tenant_id=%s', (tenant_id,))
cur.execute('SELECT id, slug, title FROM posts WHERE tenant_id=%s', (tenant_id,))
    ...
    for post_id, slug, ptitle in posts:
        if ptitle == md_title:
            ...
            if not row:
                ...
                cur.execute('INSERT INTO products (tenant_id,asin,title,price,active) VALUES (%s,%s,%s,%s,true) RETURNING id', (tenant_id,asin,pname,pnum))
                pid = cur.fetchone()[0]
            else:
                pid = row[0]
...
cur.execute('SELECT title FROM posts WHERE tenant_id=%s AND product_id IS NULL', (tenant_id,))
print('\nSEM ASIN:')
for r in cur.fetchall():
    print(f'  {r[0][:70]}')

# Verificar panelas
cur.execute("SELECT p.title, pr.title, pr.asin, pr.price FROM posts p LEFT JOIN products pr ON p.product_id=pr.id AND p.tenant_id=pr.tenant_id WHERE p.tenant_id=%s AND lower(p.title) LIKE '%panela%'", (tenant_id,))
print('\nVerificacao panelas:')
for r in cur.fetchall():
    prod = r[1] or 'N/A'
    asin = r[2] or ''
